...
def main(args):
    ...
    # Query the first 5 rows (should be empty)
    with engine.connect() as conn:
        result = conn.execute(text(f"SELECT count(*) FROM {table};")).fetchall()
        print("\nNumber of lines:")
        print(result)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--parse_dates', nargs='+', default=None, type=str, help='Datetime columns "[name_1, name_2, ...]"')
